...
void EspUsbHost::_onReceive(usb_transfer_t *transfer) {
  ...
  if (endpoint_data->bInterfaceClass == USB_CLASS_HID) {
    if (endpoint_data->bInterfaceSubClass == HID_SUBCLASS_BOOT) {
      ...
      // the keycode array holds up to 6 keycodes for up to 6 keys being pressed simultanously ( 6-key roll-over )
      ...
      // usbHost->onKeyboardKey(usbHost->getKeycodeToAscii(report-keycode, shift), report-keycode, makeBreak, report-modifier);
          ...
          if (report.modifier != last_report.modifier) { // here report.modifier != last_report.modifier => modifier key change
            if (report.modifier > last_report.modifier) {
              uint8_t modifier_make = report.modifier - last_report.modifier;
              usbHost->onKeyboardKey(usbHost->getKeycodeToAscii(report.keycode[0], shift), 0, 1, modifier_make);
            }
            else {
              uint8_t modifier_break = last_report.modifier - report.modifier;
              usbHost->onKeyboardKey(usbHost->getKeycodeToAscii(report.keycode[0], shift), 0, 0, modifier_break);
            }
          }
          else {  // here report.modifier == last_report.modifier => non modifier key change
            uint8_t i=5;
            bool new_keycode = false;
            while (( i >= 0 ) && ( !new_keycode )) {  // look up the most recent keycode change
              if (report.keycode[i] != last_report.keycode[i]) {
                new_keycode = true;
              }
              i--;
            }
            i++;
            if (new_keycode) {    // keycode change found
              if (report.keycode[i] == 0) {     // keycode for Make ( key pressed ) ?
                usbHost->onKeyboardKey(usbHost->getKeycodeToAscii(last_report.keycode[i], shift), last_report.keycode[i], 1, 0);
              }
              else {                            // keycode for Break ( key released )
                usbHost->onKeyboardKey(usbHost->getKeycodeToAscii(report.keycode[i], shift), report.keycode[i], 0, 0);
              }
            }
          }
...
void EspUsbHost::onKeyboardKey(uint8_t ascii, uint8_t keycode, uint8_t makeBreak, uint8_t modifier) {
  if (' ' <= ascii && ascii <= '~') {
    // printable
